...
export function TeamStats() {
  const { teamStats, matchResults, monthlyStats, possessionData } = teamData;

  const getResultBadgeClass = (result) => {
    const classes = {
      'W': 'bg-green-900 text-green-300',
  ...
  const getResultDotClass = (result) => {
    const classes = {
      'W': 'bg-green-500',
      'L': 'bg-red-500',
      'D': 'bg-yellow-500'
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-gray-800 border-gray-700">
          <CardContent className="p-6">
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-400">{teamStats.goalsScored}</div>
        ...
        <CardContent>
          <div className="space-y-4">
            {matchResults.map((match) => (
              <div key={match.id} className="flex items-center justify-between p-4 bg-gray-900 rounded-lg">
                <div className="flex items-center gap-4">
                  <div className={`w-3 h-3 rounded-full ${getResultDotClass(match.result)}`}></div>
                ...
                <div className="flex items-center gap-6 text-sm">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${getResultBadgeClass(match.result)}`}>
